...
class UNetDecoderBlock(nn.Module):
    ...
    def forward(self, x, skip_connection):
        # Resize first, then staple the skip connection
        x = self.deconv(x)
...
class StemExtractorUNet(nn.Module):
    def __init__(self, num_stems=4):
        ...
        # ENCODER (Compressing: 512 -> 256 -> 128 -> 64 -> 32 -> 16)
        ...
        # DECODER (Expanding: 16 -> 32 -> 64 -> 128 -> 256 -> 512)
        # Args: in_channels, skip_channels, out_channels
    ...
    def forward(self, x):
        ...
        # Decoder Pass (with matching spatial skip connections)
        d5 = self.dec5(b, e4)
        d4 = self.dec4(d5, e3)
        d3 = self.dec3(d4, e2)
        d2 = self.dec2(d3, e1)
